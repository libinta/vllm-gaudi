...
run_tests_for_model() {
  ...
  for i in $(seq 0 $((NUM_PREFILL_INSTANCES-1))); do
    ...
    BASE_CMD="HABANA_VISIBLE_DEVICES=0 UCX_TLS=tcp MY_ROLE=PREFIL VLLM_NIXL_SIDE_CHANNEL_PORT=$SIDE_CHANNEL_PORT vllm serve $model_name \
    --port $PORT \
  ...
  for i in $(seq 0 $((NUM_DECODE_INSTANCES-1))); do
    ...
    # Build the command with or without model-specific args
    BASE_CMD="HABANA_VISIBLE_DEVICES=1 UCX_TLS=tcp MY_ROLE=DECODE VLLM_NIXL_SIDE_CHANNEL_PORT=$SIDE_CHANNEL_PORT vllm serve $model_name \
    --port $PORT \
    --gpu-memory-utilization 0.3 \
...
#        }'
 ...
 #      }'
  ...
  python3 /root/litang/github/llama/official/vllm-gaudi/benchmarks/benchmark_serving.py \
   --port 9192 \
   --seed "$(date +%s)" \
   --model /root/software/data/pytorch/huggingface/hub/models--meta-llama--Llama-3.1-8B-Instruct/snapshots/0e9e39f249a16976918f6564b8830bc894c89659/ \
   --dataset-name random \
